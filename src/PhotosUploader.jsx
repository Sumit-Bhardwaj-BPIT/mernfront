...
const PhotosUploader = ({addedPhotos,setAddedPhotos}) => {
    ...
    async function addPhotobyLink(ev) {
      ev.preventDefault();
      const { data: filename } = await axios.post('https://airnbback.onrender.com/upload-by-link', { link: photoLink });
      setAddedPhotos((prev) => {
    ...
    function uploadPhoto(ev) {
      ...
      axios.post('https://airnbback.onrender.com/upload', data, {
    ...
    if (typeof link === 'string') {
      ...
      return (
        <div className="rounded-2xl relative" key={index}>
          <img src={`https://airnbback.onrender.com/uploads/${filename}`} alt="" />
